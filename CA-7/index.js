...
app.get("/", async (req, res) => {
  try {
    ...
    const data = await con.db("newDB").collection("people").find().toArray(); // atlieku veiksmus duomenų bazėje
...
app.post("/", async (req, res) => {
  try {
    const user = { name: "Danute", surname: "Vainiene", age: "62" };
    const con = await client.connect();
    const data = await con.db("newDB").collection("people").insertOne(user);
    await con.close();
    res.send(data);
  } catch (error) {
    res.status(500).send({ error });
